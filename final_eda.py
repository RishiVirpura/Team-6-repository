...
Original file is located at
    https://colab.research.google.com/drive/10mJNB9sUZNRl8Vzdcj_4RSaSxndwPnlS
...
#Removing columns that contain numerator, denominator, confidence intervals, and other features associated with specific measure IDs
...
#filtering out rows where county_ranked = 1, taking into account county level for analysis
data = data[data['county_ranked']==1]
data

"""# Checking for missing values"""

#checking for null values throughout the columns
pd.set_option('display.max_rows', 500)
data.isnull().sum().sort_values(ascending=False)

data.shape

#this is to filter columns out based on the number of null values it has
...
#filtering the columns where correlation value with the target variable is greater than 0.1
data_corr = data.corr()
...
#we do not want to impute the our target variable, therefore dropping rows where the tagret variable is null
data = data[data['v005_rawvalue'].isnull()==False]
data

#saving columns where there are still missing values remaining
...
for col in missing_list:
  # compute mean and std of columns with missing values
    ...
    # generate `num_na` samples from distribution
...
plt.scatter(data['Physical inactivity raw value'], data['Preventable hospital stays raw value'])
plt.xlabel('Physical inactivity raw value')
plt.ylabel('Preventable hospital stays raw value')
plt.title("Relationship between Preventable hospital stays and Physical inactivity")
m, b = np.polyfit(data['Physical inactivity raw value'], data['Preventable hospital stays raw value'], 1)

#add linear regression line to scatterplot 
plt.plot(data['Physical inactivity raw value'], m*data['Physical inactivity raw value']+b)

plt.figure(figsize=(10, 10))
plt.scatter(data['Premature age-adjusted mortality raw value'], data['Preventable hospital stays raw value'])
plt.xlabel('Premature age-adjusted mortality raw value')
plt.ylabel('Preventable hospital stays raw value')
plt.title("Relationship between Preventable hospital stays and Premature age-adjusted mortality")
...
#Selecting feature class and target class
...
#running the model with hyperparameters and analyzing them based on performance metrics like RMSE and R2
...
#Selecting the best model and use it with our test dataset 
...
#feature selection based on the coefficient values in the model
